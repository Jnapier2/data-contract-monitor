...
12. The reader plugin entry-point API is intentionally narrow and pre-stable in v0.3.3; third-party readers remain responsible for obeying resource/security contracts.
...
17. Cloud warehouse/object-store connectors, dbt/OpenLineage emitters, organization-level roles, and AI-assisted contract authoring are not included in v0.3.3; the reader/plugin and contract lifecycle boundaries are intended to support later additions without coupling them to the core validator.
